...
# 自分のプロジェクト権限を確認（書き込み前にこれで EDIT を確かめる）。
# 最低 VIEW が要る: アクセス権の無い project では accessLevel:null ではなく 403 が返る。
curl -s "${H[@]}" $BASE/projects/<projectId>/my-access     # MCP: project_my_access
# → { "accessLevel": "EDIT" | "VIEW" }  （VIEW なら書き込みは 403。アクセス権なしは 403）
```
...
### 3.1 Mermaid から関係性マップを取り込む / 領域分け

```bash
# Mermaid 記法（erDiagram 等）をAIで解析して冪等にインポート。
# 内部で Anthropic API を呼ぶため Anthropic APIキーが必要（未設定だと 400「Anthropic APIキーが未設定です」）。
curl -s -X POST "${H[@]}" $BASE/projects/<projectId>/data-objects/import-mermaid \
  -d '{"mermaid":"erDiagram\n  CUSTOMER ||--o{ ORDER : places\n  ORDER ||--|{ ORDER_ITEM : contains"}'
# MCP: data_object_import_mermaid { "projectId":"<projectId>", "mermaid":"erDiagram\n ..." }

# オブジェクトを領域（サブプロジェクト）に紐付け（null で解除）
curl -s -X PUT "${H[@]}" $BASE/data-objects/<objId>/sub-project \
  -d '{"subProjectId":"<subProjectId>"}'
# MCP: data_object_set_sub_project { "id":"<objId>", "subProjectId":"<subProjectId>" }

# SCOPE 注釈（領域枠）を描いて、枠内のオブジェクトを一括で領域紐付け
curl -s -X POST "${H[@]}" $BASE/projects/<projectId>/data-object-annotations \
  -d '{"kind":"SCOPE","subProjectId":"<subProjectId>","positionX":0,"positionY":0,"width":600,"height":400,"borderStyle":"dashed","fillOpacity":0.1}'
# → 戻りの id（注釈ID）を使って枠内を一括紐付け
curl -s -X POST "${H[@]}" $BASE/data-object-annotations/<annotationId>/apply-scope-links
# MCP: data_object_annotation_create { ..., "kind":"SCOPE" } → data_object_apply_scope_links { "id":"<annotationId>" }
```
...
```jsonc
// 全302ルートをタグ別に探索（domain で部分一致フィルタ）
...
明示権限が無いときは会社ロール由来の既定にフォールバックする。**閲覧専用（VIEW）で書き込むと 403**。
members 系・操作履歴は **管理者（会社 OWNER/ADMIN または全体管理者）限定**。

```bash
# 自分の実効権限（最低 VIEW が必要。アクセス権の無い project では null でなく 403）
curl -s "${H[@]}" $BASE/projects/<projectId>/my-access      # MCP: project_my_access
# → { "accessLevel": "EDIT" | "VIEW" }（触れない project は 403。返り値 null は VIEW以上を別経路で持つ稀なケースのみ）

# メンバー一覧＋実効権限（管理者限定）。set/remove の userId はここから取る
curl -s "${H[@]}" $BASE/projects/<projectId>/members        # MCP: project_member_list
# → [{ userId, email, name, orgRole, explicitLevel, effectiveLevel }, ...]

# 明示権限を設定（upsert。管理者限定）
curl -s -X PUT "${H[@]}" $BASE/projects/<projectId>/members/<userId> \
  -d '{"accessLevel":"VIEW"}'                                # MCP: project_member_set
# → 編集可に戻すなら {"accessLevel":"EDIT"}

# 明示権限を削除して既定に戻す（管理者限定）
curl -s -X DELETE "${H[@]}" $BASE/projects/<projectId>/members/<userId>   # MCP: project_member_remove

# 操作履歴（監査ログ。body 含む。管理者限定。一般メンバーは 403）
curl -s "${H[@]}" "$BASE/projects/<projectId>/change-logs?limit=100"      # MCP: change_log_list
```

---

## 10. 非同期AIジョブ（enqueue → poll）

重いAI生成（Mermaid 生成・KPI 生成・イシュー候補生成など）は同期で待たず、ジョブとして起票してポーリングする。
本番は Upstash QStash 経由で非同期実行、ローカルは enqueue 内で inline 実行。

```bash
# 1) ジョブ起票（type 必須。payload は種別ごとの入力。秘匿情報は入れない）
#    AI_MERMAID_OBJECTMAP / AI_MERMAID_FLOW は payload.mermaid（既存の Mermaid テキスト）が必須。
#    どちらも「Mermaid を生成する」ジョブではなく「与えた Mermaid を AI 解析する」処理。
#    payload.mermaid を渡さないと "payload.mermaid が必要です" で FAILED になる。
curl -s -X POST "${H[@]}" $BASE/projects/<projectId>/ai-jobs \
  -d '{"type":"AI_MERMAID_OBJECTMAP","payload":{"mermaid":"erDiagram\n  CUSTOMER ||--o{ ORDER : places"}}'
# MCP: ai_job_enqueue { "projectId":"<projectId>", "type":"AI_MERMAID_OBJECTMAP", "payload":{ "mermaid":"erDiagram\n ..." } }
# → { "jobId":"<jobId>", "status":"QUEUED" }
#   type と payload:
#     AI_MERMAID_OBJECTMAP { mermaid } → 解析して関係性マップに取り込み（永続）。result {kind:"OBJECT_GRAPH", graph}
#     AI_MERMAID_FLOW      { mermaid } → 解析して業務フロー構造に変換（永続せず result {kind:"MERMAID_FLOW", flow}）
#     AI_KPI               { category, ... }
#     AI_ISSUE_SUGGEST     { context: {...} }

# 2) 完了までポーリング（status が SUCCEEDED / FAILED になるまで数秒間隔）
curl -s "${H[@]}" $BASE/jobs/<jobId>                         # MCP: ai_job_get { "id":"<jobId>" }
# → { "id", "type", "status":"SUCCEEDED", "result": { ... } }

# 3) 直近ジョブ一覧
curl -s "${H[@]}" "$BASE/projects/<projectId>/jobs?limit=20" # MCP: ai_jobs_list

# 4) AI_MERMAID_OBJECTMAP はジョブ内で取り込みまで完了している（result {kind:"OBJECT_GRAPH", graph}）。
#    result を改めて import-mermaid に渡すと二重取り込みになるので渡さない。取り込み結果は data_object_list で確認:
curl -s "${H[@]}" $BASE/projects/<projectId>/data-objects    # MCP: data_object_list
```
