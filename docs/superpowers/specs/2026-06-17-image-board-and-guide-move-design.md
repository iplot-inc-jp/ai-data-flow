...
## 追記（2026-06-17 実装時の方針変更）— キャンバスは Excalidraw(MIT) 埋め込みに変更
ユーザー判断で「自作SVG」→ **Excalidraw(`@excalidraw/excalidraw`, MIT) 埋め込み**に切替（理由: 図形/矢印/テキスト/画像/手書き/undo-redo/PNG・SVG出力が即完成し、ラフ下書きという用途に手描き調UIが適する）。tldraw は商用 $6,000/年＋透かしのため不採用（GPLv3ガント回避と同方針）。
- **データモデル変更**: 要素テーブル(`ImageBoardElement`)は廃止。`ImageBoard` に **`scene Json?`**（Excalidraw の `{elements, appState, files}`）を持たせ、ボード単位で保存。`ImageBoardKind(ASIS|TOBE)` は維持。Attachment 逆リレーションも撤去（画像は当面 Excalidraw 標準の貼付/D&D。共有プール連携は後追い）。
- **API**: `GET/POST /api/projects/:projectId/image-boards`（一覧[kindフィルタ・scene除外の軽量]／作成）、`GET/PATCH/DELETE /api/image-boards/:id`（取得[scene込]／更新[title/kind/order/scene]／削除）。:id 系は board→projectId をロードして ProjectAccess を assert（overview-matrix と同型）。
- **フロント**: `lib/image-board.ts`、`image-board/page.tsx`（ASIS/TOBEタブ＋ボード一覧＋ホスト＋タイトル編集＋保存状態）、`_components/ExcalidrawBoard.tsx`（`next/dynamic` ssr:false で動的読込、`initialData`=scene、onChange を getSceneVersion で変化判定→debounce 900ms で PATCH scene、`viewModeEnabled=!canEdit`、CSS=`@excalidraw/excalidraw/index.css`）。サイドバー「現状把握」に Presentation アイコンで追加。
- **検証済**: backend nest build / db push / ライブCRUD smoke 全緑（create→get→patch scene/title→list[軽量]→delete 204→404、unauth 401）。frontend tsc 0 / vitest 223 / next build（image-board は Excalidraw を別チャンク動的読込で First Load 124kB）。
