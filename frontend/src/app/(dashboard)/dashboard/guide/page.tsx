 ...
 * SVG のフロー図と各ステップの説明 + 機能の解説で案内する。
 *
 * このページは特定プロジェクトに依存しない汎用マニュアルなので、
 * トップレベル（/dashboard/guide）に置く。各機能はプロジェクトを選んでから
 * サイドバーで開くため、ここではプロジェクト個別の deep-link は持たない。
 */
...
// ===== 各ステップの説明 + 機能の解説 =====
// 汎用マニュアルなのでプロジェクト個別のリンクは持たず、機能名と説明のみを示す。
// 実際の操作はプロジェクトを開いてサイドバーの各メニューから行う。
type GuideFeature = {
  name: string
  description: string
  icon: LucideIcon
...
// 機能の説明カード（汎用マニュアルなのでリンクは持たない）
function GuideFeatureCard({ feature }: { feature: GuideFeature }) {
  return (
    <div className="blueprint-card flex items-start gap-3 p-4">
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="このツールの全体の流れ（ASIS → TOBE → GAP → 課題ツリー → タスク）と、各ステップで使う機能を案内します。各機能の操作はプロジェクトを開いてサイドバーから行います。"
      />
        ...
        <section key={section.badge} className="space-y-3">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {section.features.map((feature) => (
              <GuideFeatureCard key={feature.name} feature={feature} />
            ))}
          </div>
          {/* このステップの操作手順（番号付き） */}
