 ...
 * CSS は ../frappe-gantt.vendor.css（dist 由来のベンダリング済み）を使うため、
 * オリジナルの `import './styles/gantt.css'` は削除している。
 */
...
export default class Gantt {
    ...
    // 変更（vendor）: upstream に dispose API が無く、document に登録した
    // mouseup リスナーがインスタンスごとに蓄積・リークしていたため追加。
    // React 側（FrappeGantt.tsx）のアンマウント時に呼ぶ。
    destroy() {
        for (const fn of this.$destroy_fns) {
            try {
                fn();
            } catch (e) {
                console.error('frappe-gantt destroy cleanup failed:', e);
            }
        }
        this.$destroy_fns = [];
    }
    ...
    refresh(tasks) {
        // 変更（vendor）: 以前は再描画後に scroll_to（既定 'today'）への自動スクロール
        // が走り、保存→refresh のたびに視界が左右へ飛んでいた。
        // 再描画前の「左端に見えている位置」を覚えておき、gantt_start のシフト分を
        // 補正したうえで同じ位置に戻す。
        ...
        if (old_gantt_start && this.gantt_start) {
            // 変更（vendor）: date_utils.diff の month 計算は upstream 由来の
            // `+ date.getDate()/31` 補正で同一日付でも 0.03 を返すため、
            // Month ビューで refresh のたびに scrollLeft が数 px ずつドリフトする。
            // gantt_start は必ず unit の整数倍でしか動かないので整数に丸める。
            const offset_units = Math.round(
                date_utils.diff(
                    old_gantt_start,
                    this.gantt_start,
                    this.config.unit,
            ...
            this.$container.scrollLeft =
                old_scroll_left + offset_units * this.config.column_width;
        } else {
            this.$container.scrollLeft = old_scroll_left;
        }
    }
    ...
    render() {
        ...
        this.set_dimensions();
        // 変更（vendor）: scroll_to === null のときは自動スクロールしない
        // （refresh / 無限パディング連結などの再描画で視界が飛ばないように）。
        if (this.options.scroll_to !== null)
    ...
    set_scroll_position(date) {
        ...
        // Recalculate
        // 変更（vendor）: $el が見つからない場合に例外で落ちないよう null ガード。
        this.current_date = date_utils.add(
            this.gantt_start,
    ...
    bind_bar_events() {
        ...
        // 変更（vendor）: ドラッグ中の共通バー更新処理。
        // mousemove とオートスクロールの両方から呼ぶ。スナップはせず生の dx で
        // 滑らかに動かし、スナップ（日付単位への丸め）はドロップ確定時に行う。
        const update_bars_from_dx = (dx) => {
            bars.forEach((bar) => {
                const $bar = bar.$bar;
                $bar.finaldx = dx;
                this.hide_popup();
        ...
        // 変更（vendor）: ドラッグ中のオートスクロール。
        // ポインタがコンテナの左右端 AUTOSCROLL_EDGE px 以内に来たら一定速度で
        // ゆっくりスクロールし、スクロールした分だけバーも追従させて
        // ポインタ直下に保つ（requestAnimationFrame ループ）。
        let autoscroll_raf = null;
        let last_client_x = null;
        const AUTOSCROLL_EDGE = 40;
        const AUTOSCROLL_SPEED = 10; // px / frame

        const autoscroll_step = () => {
            autoscroll_raf = null;
            if (!action_in_progress() || last_client_x === null) return;
            // 変更（vendor）: 実ドラッグが始まる（mousemove で 10px 超動いて
            // bar_being_dragged === true になる）まではスクロールしない。
            // これが無いと、コンテナ左右端 40px 以内のバーを押下保持しただけで
            // 毎フレームスクロール＋バー移動が起き、mouseup で意図しない
            // date_change（日付変更の保存）まで発火してしまう。
            if (this.bar_being_dragged !== true) {
                autoscroll_raf = requestAnimationFrame(autoscroll_step);
                return;
            }
            const rect = this.$container.getBoundingClientRect();
            let delta = 0;
            if (last_client_x >= rect.right - AUTOSCROLL_EDGE)
                delta = AUTOSCROLL_SPEED;
            else if (last_client_x <= rect.left + AUTOSCROLL_EDGE)
                delta = -AUTOSCROLL_SPEED;
            if (delta !== 0) {
                const before = this.$container.scrollLeft;
                this.$container.scrollLeft = before + delta;
                if (this.$container.scrollLeft !== before) {
                    // SVG 座標系でのポインタ位置 = clientX - コンテナ左端 + scrollLeft
                    const svg_x =
                        last_client_x - rect.left + this.$container.scrollLeft;
                    update_bars_from_dx(svg_x - x_on_start);
                }
            }
            autoscroll_raf = requestAnimationFrame(autoscroll_step);
        };
        const start_autoscroll = () => {
            if (autoscroll_raf === null)
                autoscroll_raf = requestAnimationFrame(autoscroll_step);
        };
        const stop_autoscroll = () => {
            if (autoscroll_raf !== null) {
                cancelAnimationFrame(autoscroll_raf);
                autoscroll_raf = null;
            }
            last_client_x = null;
        };
        ...
        $.on(this.$svg, 'mousedown', '.bar-wrapper, .handle', (e, element) => {
            ...
            // 変更（vendor）: ドラッグ中フラグ（bar.date_changed の発火抑制）と
            // 端オートスクロールの開始。
            this.dragging_in_progress = true;
        ...
        if (this.options.infinite_padding) {
            ...
            $.on(this.$container, 'mousewheel', (e) => {
                ...
                if (!extended && e.currentTarget.scrollLeft <= trigger) {
                    ...
                    this.setup_date_values();
                    // 変更（vendor）: 再描画中に scroll_to（today）への自動スクロールが
                    // 走って瞬間移動しないよう、一時的に無効化する。
                    const old_scroll_op = this.options.scroll_to;
                    this.options.scroll_to = null;
                    this.render();
                    this.options.scroll_to = old_scroll_op;
        ...
        $.on(this.$svg, 'mousemove', (e) => {
            ...
            // 変更（vendor）: ドラッグ中はスナップせず生の dx で滑らかに動かす。
            // スナップはドロップ確定（mouseup）時に行う。
            last_client_x = e.clientX;
            const dx = (e.offsetX || e.layerX) - x_on_start;
            update_bars_from_dx(dx);
        });

        // 変更（vendor）: ドロップ確定は document の mouseup で行う（SVG の外で
        // マウスを離しても確定するように）。ここで初めてビューモードの日付単位へ
        // スナップし、date_change を 1 回だけ発火する。
        // リスナーはインスタンス全体をクロージャで保持するため、destroy() で
        // 解除できるよう $destroy_fns に解除関数を登録する（rAF も止める）。
        const on_document_mouseup = () => {
            ...
            moved_bars.forEach((bar) => {
                const $bar = bar.$bar;
                // 実際に画面へ適用された移動量からスナップ量を求める
                // （依存バーより左へは動けない等の制限で生 dx と一致しないことがある）。
                const applied_dx = was_resizing_right
                    ? $bar.getWidth() - $bar.owidth
                    : $bar.getX() - $bar.ox;
                const snapped_dx = this.get_snap_position(
                    applied_dx,
                    $bar.ox,
                );
                ...
                if (was_resizing_left) {
                    if (parent_bar_id === bar.task.id) {
                        bar.update_bar_position({
                            x: $bar.ox + snapped_dx,
                            width: $bar.owidth - snapped_dx,
                        });
                    } else {
                        bar.update_bar_position({ x: $bar.ox + snapped_dx });
                    }
                } else if (was_resizing_right) {
                    if (parent_bar_id === bar.task.id) {
                        bar.update_bar_position({
                            width: $bar.owidth + snapped_dx,
                        });
        ...
        this.$destroy_fns.push(() => {
            document.removeEventListener('mouseup', on_document_mouseup);
            // アンマウントが押下保持中に起きた場合に rAF ループが
            // 永久に回り続けないよう、オートスクロールも止める。
            stop_autoscroll();
        });
    ...
    get_snap_position(dx, ox) {
        ...
        // 変更（vendor）: 旧実装はゼロ方向への切り捨てで偏っていたため、
        // 最寄りのスナップ位置への四捨五入に変更（ドロップ時スナップが自然になる）。
        const snap_px = this.config.column_width / unit_length;
        let final_dx = Math.round(dx / snap_px) * snap_px;
